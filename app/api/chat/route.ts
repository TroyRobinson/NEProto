  ...
  const tools = [
    ...
    },
    {
      ...
      function: {
        name: 'load_stat',
        description:
          'Load a stored statistic by code (use a variable id like B01003_001E from the active metrics list). Returns ZCTA value data.',
        parameters: {
          type: 'object',
          properties: {
            code: {
              type: 'string',
              description: 'Statistic code (variable id from active metrics list)',
            },
          },
          required: ['code'],
        },
...
export async function POST(req: NextRequest) {
  ...
  if (stats && stats.length) {
    ...
    messages.splice(1, 0, {
      role: 'assistant',
      content: `Active metrics (use load_stat with a code to get values):\n${summary}`,
    });
    const dataBlock = stats
      .map(
        (s: { code: string; data?: Record<string, unknown> }) =>
          `${s.code}: ${Object.entries(s.data || {})
            .map(([z, v]) => `${z}:${v}`)
            .join(', ')}`
      )
      .join('\n');
  ...
  if (
    first.toolInvocations.length === 0 &&
    (first.lastSearchEmpty || !first.message?.content?.trim())
  ) {
    convo.push({
      role: 'assistant',
      content: 'No direct match found. Consulting a more advanced model...',
    });
