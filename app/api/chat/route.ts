      ...
      if (!toolCalls.length) {
        ...
        if (!message?.content?.trim()) {
          ...
          if (lastSearch?.length) {
            const top = lastSearch[0];
            if (await validateVariableId(top.id, year, dataset)) {
              toolInvocations.push({ name: 'add_metric', args: { id: top.id, label: top.label } });
              return NextResponse.json({
                message: {
                  role: 'assistant',
                  content: `Added "${top.label}" to your metrics list.`,
                },
                toolInvocations,
              });
            }
