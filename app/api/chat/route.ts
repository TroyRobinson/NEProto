...
export function isConceptualQuestion(input: string): boolean {
  return /\b(why|how|compare|contrast|explain|insight|analysis)\b/i.test(
    input
  );
}
...
export async function POST(req: NextRequest) {
  ...
  const isConceptual = isConceptualQuestion(lastUser);
  ...
  if (isConceptual) {
    convo.push({
      role: 'assistant',
      content: 'Consulting a more advanced model for deeper reasoning.',
    });
    const deeper = await runModel('openai/gpt-5-mini', convo, year, dataset);
    toolInvocations.push(...deeper.toolInvocations);
    return NextResponse.json({ message: deeper.message, toolInvocations, usedFallback: true });
  }
