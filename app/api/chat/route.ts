...
function needsDeepReasoning(input: string): boolean {
  return /\b(why|how|explain|compare|contrast|insight)\b/i.test(input);
}
...
export async function POST(req: NextRequest) {
  ...
  } else if (needsDeepReasoning(lastUser)) {
    fallbackReason = 'a deeper explanation was requested';
  }
  if (fallbackReason) {
    convo.push(first.message as Message);
    convo.push({
      role: 'assistant',
      content: `Using a smarter model because ${fallbackReason}.`,
    });
