...
export async function fetchZctaMetric(
  ...
  const cacheKey = `${dataset}-${year}-${geography}-${variable}-${zctas.join(',')}`;
  ...
  let geoParam = geography;
  if (geography === 'zip code tabulation area') {
    geoParam = 'zip%20code%20tabulation%20area';
  } else if (geography === 'zip code' || geography === 'zipcode') {
    geoParam = 'zipcode';
  } else {
    geoParam = geography.replace(/\s+/g, '%20');
  }

  const res = await fetch(
    `https://api.census.gov/data/${year}/${dataset}?get=${variable}&for=${geoParam}:${zctas.join(',')}`
  );
