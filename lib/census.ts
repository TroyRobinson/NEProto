...
export async function fetchCensusMetric(
  variable: string,
  ...
  await log({
    service: 'US Census',
    direction: 'request',
    message: { type: 'metric', variable, year, dataset, geography },
  });

  const res = await fetch(
    `https://api.census.gov/data/${year}/${dataset}?get=${variable}&for=${encodeURIComponent(geography)}:${zctas.join(',')}`
  );
