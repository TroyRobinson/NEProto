...
interface CensusChatProps {
  onAddMetric: (metric: { id: string; label: string }) => Promise<boolean>;
      ...
      if (data.toolInvocations) {
        for (const inv of data.toolInvocations) {
          if (inv.name === 'add_metric') {
            const ok = await onAddMetric(inv.args);
            if (!ok) {
              setMessages((prev) => [
                ...prev,
                {
                  role: 'assistant',
                  content: `No data available for ${inv.args.label} at this geography.`,
                },
              ]);
            }
