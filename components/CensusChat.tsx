...
export default function CensusChat({ onAddMetric, onLoadStat, onClose }: CensusChatProps) {
  ...
  const clearChat = () => {
    ...
    const sendMessage = async () => {
      ...
      setLoading(true);
      const systemPrompt = `You help users find US Census statistics. Limit responses to ${config.region} using ${config.dataset} ${config.year} data for ${config.geography}. Respond concisely in plain text with no markdown.`;
